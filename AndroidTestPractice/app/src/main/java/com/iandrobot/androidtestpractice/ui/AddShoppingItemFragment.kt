...
class AddShoppingItemFragment @Inject constructor(
    val glide: RequestManager // this is provided by the factory, see ShoppingFragmentFactory
        ...
        viewModel.insertShoppingItemStatus.observe(viewLifecycleOwner, Observer {
            it.getContentIfNotHandled()?.let { result ->
                when (result.status) {
                    ...
                    Status.ERROR -> {
                        Snackbar.make(requireActivity().rootLayout, "An unknown error occurred", Snackbar.LENGTH_LONG).show()
                    }
                    Status.LOADING -> {
                        // no op
                    }
